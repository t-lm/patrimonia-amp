...
export default function OrganiserUpdateForm(props) {
  const {
    id: idProp,
    organiser: organiserModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = organiserRecord
      ? { ...initialValues, ...organiserRecord }
      : initialValues;
    setName(cleanValues.name);
    setDescription(cleanValues.description);
    setDescription_en(cleanValues.description_en);
    setDescription_es(cleanValues.description_es);
    setDescription_de(cleanValues.description_de);
    setDescription_nl(cleanValues.description_nl);
    setWww(cleanValues.www);
    setPhone(cleanValues.phone);
    setEmail(cleanValues.email);
    setType(cleanValues.type);
    setErrors({});
  };
  const [organiserRecord, setOrganiserRecord] =
    React.useState(organiserModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Organiser, idProp)
        : organiserModelProp;
      setOrganiserRecord(record);
    };
    queryData();
  }, [idProp, organiserModelProp]);
  React.useEffect(resetStateValues, [organiserRecord]);
  const validations = {
    name: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Organiser.copyOf(organiserRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "OrganiserUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || organiserModelProp)}
